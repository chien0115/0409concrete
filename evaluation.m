...
for i = 1:x1  % 遍歷每個染色體
    ...
    for k = 1:num_dispatch_order %派遣順序工地組合
        ...
        %dispatch_order_for_chromosome(1:k-1)從派遣順序的開始到當前位置(k-1)的所有派遣記錄
        %find(..., 1, 'last') 1：只要找一個位置 'last'：要最後一個符合的位置
        


        %第一次的if isempty(previous_work_idx)是要用來決定什麼時候開始工作
        ...
        %第二部分計算等待時間和相應的懲罰
        % 计算等待时间
        if ~isempty(previous_work_idx)            % 第一次到该工地
